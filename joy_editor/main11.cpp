...
// ======================== 脚步音效 ========================
static uint32_t      gAudioDevice  = 0;       // 来自 jaudio
static Uint8*        gStepData     = nullptr; // 脚步 WAV 原始数据
static Uint32        gStepLen      = 0;
static SDL_AudioSpec gStepSpec     = {};
static float         gStepTimer    = 0;      // 脚步计时器
static const float   STEP_INTERVAL = 0.30f;  // 脚步间隔（≈ 走动画每2帧）
// 预创建的音频流循环使用（避免每步创建/销毁）
static SDL_AudioStream* gStepStream = nullptr;
...
static bool app_init(void) {
    ...
    SDL_Log("audio device id = %u", gAudioDevice);
    { auto base = SDL_GetBasePath();
      std::string wav_path = std::string(base ? base : "") + "joy2d_editor_sounds/ak47.wav";
      SDL_Log("loading wav: %s", wav_path.c_str());
      if (!SDL_LoadWAV(wav_path.c_str(), &gStepSpec, &gStepData, &gStepLen))
          SDL_Log("load footstep wav FAILED: %s", SDL_GetError());
      else {
          SDL_Log("footstep wav OK: freq=%d fmt=0x%x ch=%d len=%u",
                  gStepSpec.freq, gStepSpec.format, gStepSpec.channels, gStepLen);
          // 预先创建一次音频流并绑定设备，之后只需 PutData 即可播放
          gStepStream = SDL_CreateAudioStream(&gStepSpec, NULL);
          if (gStepStream) {
              SDL_BindAudioStream(gAudioDevice, gStepStream);
              SDL_SetAudioStreamGain(gStepStream, 0.2f); // 音量调低到 20%
              SDL_Log("footstep audio stream created and bound");
          } else {
...
static void play_footstep() {
    if (!gStepData || !gStepStream) return;
    // 先清空流中残留的旧数据再推新数据
    SDL_ClearAudioStream(gStepStream);
    if (SDL_PutAudioStreamData(gStepStream, gStepData, gStepLen))
        SDL_Log("footstep played"); // 调试: 确认触发了
    else
        SDL_Log("footstep PutData FAILED: %s", SDL_GetError());
}
...
static void app_quit(void) {
    ...
    // 清理音频
    if (gStepStream) { SDL_UnbindAudioStream(gStepStream); SDL_DestroyAudioStream(gStepStream); }
    if (gStepData) SDL_free(gStepData);
