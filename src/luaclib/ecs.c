...
static int lecs_get(lua_State* L)
{
        ...
        id = (ecs_id_t)luaL_checkinteger(L, 2);
        name = luaL_checkstring(L, 3);
        size = ecs_get(world, id, name, &ptr);
        if (size <= 0) {
                lua_pushnil(L);
        }
        else {
...
static int lecs_query(lua_State* L)
{
        ...
        id_arr = ecs_queryx(world, components_count, component_names);
        args_count = vector_size(kecs, &id_arr);
        if (args_count > 1) {
                int kkk = 0;
        }
        lua_createtable(L, args_count, 0);
        for (int i = 0; i < args_count; i++) {
                lua_pushinteger(L, vector_a(kecs, &id_arr, i));
                lua_rawseti(L, -2, i + 1); 
        }
