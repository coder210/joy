...
static std::list<int> inputs;   // 可保留用于积累输入（但当前版本直接发送）
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
	...
	world.system<LogicVelocity, Player>()
		.interval(0.02f)
		.each([](LogicVelocity& v, Player& pr) {
		for (auto rit = inputs.rbegin(); rit != inputs.rend(); ++rit) {
			int input = *rit;
			if (INPUT_UP == input) {
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
	...
	// 如果需要从 inputs 列表批量发送，可以在这里处理
	// 但当前已在按键事件中直接发送，故 inputs 列表暂不使用
	/*
	while (!inputs.empty()) {
