...
#include "stb_image.h"
#include <chrono>
#include <cmath>    // ���� sinf, cosf, tanf, sqrtf
...
GPUShaderBundle createSDLGPUShaderBundle() {
   /*     const char* dir = "shaders";
        ...
        bundle.vertex =
                loadSDLGPUShader("joy2d_editor_shaders/vert.spv",SDL_GPU_SHADERSTAGE_VERTEX, 0, 1);
        bundle.fragment = loadSDLGPUShader("joy2d_editor_shaders/frag.spv", 
...
void createImageTexture() {
        ...
        // ʹ�� SDL_LoadFile ��ȡ�ļ�����
        size_t file_size = 0;
        void* file_data = SDL_LoadFile("joy2d_editor_textures/logo.png", &file_size);
        if (!file_data) {
                SDL_Log("Failed to load file: joy2d_editor_textures/logo.png");
                return;
        }
...
void updateMVPData() {
       ...
       /* gMVP.model = Mat4::rotateY(radians(gRotateY)) *
                     ...
                     gMVP.model;*/
        gMVP.model = Mat4::translate(cubePositions[0]);  // ƽ��
